...
def issue_task():
    // тут должна бьыть логика брать таску которая еще не isSend с limit 1 ( и возможно с  order by id )
    // если таких нет то все задания выданы скипаем

    // если есть то помечаем эту таску isSend  а по вебсокету как-то отдавать всегда все  isSend
    // отправляем задание наврено по веь сокету отдельнйо ручкой на актуальные таски???
